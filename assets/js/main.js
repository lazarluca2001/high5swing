...
function setTheme(mode) {
  ...
  // Szinkronizáljuk a választókat az oldalon
...
function injectSidebarLayout() {
  // Naptár oldalon a HTML-ben már benne van a sidebar
...
function renderResultItem(it) {
  const fieldSize = hasValue(getRoleFieldSize(it.roleNorm, it.leaderCount, it.followerCount)) ? safeText(getRoleFieldSize(it.roleNorm, it.leaderCount, it.followerCount)) : "";
  const stats = [];
    ...
    <details class="result-card">
      <summary class="result-summary">
        ...
        <div class="result-right">
          <div class="result-col"><div class="result-label">Helyezés</div><div class="result-value">${hasValue(it.final) ? it.final : "—"}</div></div>
          <div class="result-col"><div class="result-label">Pont</div><div class="result-value">${hasValue(it.points) ? it.points : "0"}</div></div>
        </div>
      </summary>
      <div class="result-body event-body">${stats.length ? `<div class="event-stats">${stats.join("")}</div>` : `<p class="tiny">Nincs további részlet.</p>`}</div>
    </details>`;
}
...
function renderProfile(groups) {
  ...
  for (const div in groups) for (const role in groups[div]) allRolesSet.add(role);
  ...
  root.innerHTML = sortedRoles.map(roleKey => {
    const roleSections = sortedDivisions.map(divKey => {
      ...
      return `<section class="block"><h2 class="block-title">${items[0].divisionOriginal}</h2><div class="result-list">${items.map(renderResultItem).join("")}</div></section>`;
    }).join("");
    return roleSections ? `<section class="role-block"><h2 class="block-title">${roleKey === "leader" ? "Leader" : "Follower"}</h2>${roleSections}</section>` : "";
  }).join("");
}
...
async function loadProfileFromSheet() {
  ...
  const root = document.getElementById("profileContent");
  if (!name) { if (root) root.innerHTML = `<div class="card"><h2>Hiányzó név</h2></div>`; return; }
  ...
  for (let r = 4; r < rows.length; r++) {
    ...
    const item = { divisionOriginal: divOrig, roleNorm, event: safeText(row[3]), date: safeText(row[4]), leaderCount: row[6], followerCount: row[7], prelim: row[8], semi: row[9], final: row[10], points: row[11], partner: row[18], _sort: parseDateToSortable(row[4]) };
    
    groups[divKey] ??= {};
    groups[divKey][roleNorm] ??= [];
    groups[divKey][roleNorm].push(item);
  }
  for (const d in groups) for (const r in groups[d]) groups[d][r].sort((a, b) => b._sort - a._sort);
...
function renderCalendar(monthIndex) {
  ...
  CAL_CONFIG.weekdays.forEach(d => {
    const div = document.createElement("div"); div.className = "weekday";
    div.textContent = d; frag.appendChild(div);
  });
  ...
  for (let i = 0; i < firstDay; i++) {
    const div = document.createElement("div"); div.className = "day empty-day-pre";
  ...
  for (let d = 1; d <= daysInMonth; d++) {
    ...
    dayEvents.forEach(e => {
      ...
      if (tagsHtml || !activeFilter) {
        ...
        card.innerHTML = `<span class="event-title">${safeText(e.Event)}</span><div class="participants-container">${tagsHtml}</div>`;
...
function setupMonthSelect() {
  ...
  sel.onchange = (e) => { currentMonthIdx = parseInt(e.target.value); updateCalendarUI(); };
}

window.changeMonth = (delta) => { currentMonthIdx = (currentMonthIdx + delta + 12) % 12; updateCalendarUI(); };
window.goToToday = () => { currentMonthIdx = new Date().getMonth(); updateCalendarUI(); };
...
function renderMemberFilter() {
  ...
  box.querySelectorAll(".filter-btn").forEach(btn => btn.onclick = () => {
...
function updateActivityChart() {
  ...
  container.innerHTML = Object.entries(CAL_CONFIG.members).map(([name, emoji]) => {
    const count = allEvents.filter(e => CAL_CONFIG.validStatuses.some(vs => safeText(e[name]).toLowerCase().includes(vs))).length;
    return `<div class="chart-column-wrapper"><div class="chart-bar" style="height:${Math.min(80, (count / 20) * 80)}px"></div><span class="chart-label">${count}</span><span class="chart-emoji">${emoji}</span></div>`;
  }).join("");
}
...
function updateNextCountdown() {
  ...
  if (!next) { box.innerHTML = `<div class="muted">Nincs közelgő esemény.</div>`; return; }
  const diff = Math.round((next._startTs - now) / 86400000);
  box.innerHTML = `<div class="next-event-title">${safeText(next.Event)}</div><div class="next-event-countdown">${diff > 0 ? `Még ${diff} nap` : "Ma kezdődik! 🔥"}</div>`;
}

function setupCalendarSidebarToggle() {
  const btn = document.getElementById("sidebarToggle"), sb = document.getElementById("calendarSidebar");
  if (btn && sb) btn.onclick = () => { sb.classList.toggle("open"); document.body.classList.toggle("calendar-sidebar-open", sb.classList.contains("open")); };
}
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  // Téma választó inicializálása oldalsávtól függetlenül (naptár oldalon a HTML-ben van)
  ...
  if (document.getElementById("participants")) loadParticipantsFromSheet().catch(console.error);
  if (document.getElementById("profileContent")) loadProfileFromSheet().catch(console.error);
